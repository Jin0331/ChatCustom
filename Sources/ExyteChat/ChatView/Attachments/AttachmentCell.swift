...
struct AttachmentCell: View {
    ...
    var content: some View {
        AsyncImageWithURLRequestView(url: attachment.thumbnailToRequest)
    }
}

struct AsyncImageWithURLRequestView: View {

    @Environment(\.chatTheme) var theme
    let url: URLRequest

    var body: some View {
        CachedAsyncImage(urlRequest: url, urlCache: .imageCache) { imageView in
            imageView
                .resizable()
                .scaledToFill()
        } placeholder: {
